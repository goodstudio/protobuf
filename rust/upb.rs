        ...
        paste! {
            $(
                pub trait [< MapWith $t:camel KeyOps >] {
                    ...
                    fn remove<'a>(m: RawMap, key: $sized_t) -> Option<Self::Value<'a>>;
                }
                ...
                impl<'msg, V: [< MapWith $t:camel KeyOps >] + ?Sized> MapInner<'msg, $t, V> {
                    ...
                    pub fn remove<'a>(&mut self, key: $sized_t) -> Option<V::Value<'a>> {
    ...
    ($key_t:ty, $key_msg_val:expr, $key_upb_tag:expr, $trait:ident for $($t:ty, $sized_t:ty, $msg_val:expr, $from_msg_val:expr, $upb_tag:expr, $zero_val:literal;)*) => {
         $(
            impl $trait for $t {
                ...
                fn insert<'a>(m: RawMap, a: RawArena, key: $key_t, value: Self::Value<'a>) -> bool {
                    unsafe {
                        upb_Map_Set(
                            m,
                            $key_msg_val(key),
                ...
                fn remove<'a>(m: RawMap, key: $key_t) -> Option<Self::Value<'a>> {
                    let mut val = $msg_val($zero_val);
                    let removed = unsafe {
...
mod tests {
    ...
    fn str_str_map() {
        ...
        assert_that!(map.size(), eq(1));
        assert_that!(map.remove("fizz".into()).unwrap(), eq("buzz"));
        map.clear();
        assert_that!(map.size(), eq(0));
    }
    ...
    fn u64_str_map() {
        ...
        assert_that!(map.remove(1).unwrap(), eq("fizz"));
        map.clear();
        assert_that!(map.size(), eq(0));
    }
